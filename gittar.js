...
function getTarUrl(obj) {
	switch (obj.site) {
		...
		default:
			return `https://github.com/${obj.repo}/archive/refs/tags/${obj.type}.tar.gz`;
	}
}
...
exports.extract = function (file, dest, opts) {
	...
	return new Promise((res, rej) => {
		...
		if (file) {
			return mkdirp(dest).then(made => tar.extract(opts).then(_ => res(dest)).catch(rej)).catch(err => rej(err))
		}
		return rej()
	});
}